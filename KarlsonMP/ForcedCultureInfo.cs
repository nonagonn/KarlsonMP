        ...
        public static void Install()
        {
                ...
                .Where(x => x.PropertyType == CultureInfoType))
            {
                ...
                if (getMethod != null)
                    ...
                    catch (Exception ex) { KMP_Console.Log($"<color=red>Exception Patching Thread Get Method {propertyInfo.Name}: {ex}</color>"); }

                MethodInfo setMethod = propertyInfo.GetSetMethod();
                if (setMethod != null)
                    try { Loader.harmony.Patch(setMethod, PatchMethod_Set); }
                    catch (Exception ex) { KMP_Console.Log($"<color=red>Exception Patching Thread Set Method {propertyInfo.Name}: {ex}</color>"); }
            }
        }
